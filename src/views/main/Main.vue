...
<template>
  <div>
        ...
        <template #fileName="{ index, row }">
          <div class="file-item" @mouseenter="showOp(row)" @mouseleave="cancelShowOp(row)">
            <template v-if="(row.fileType == 3 || row.fileType == 1) && row.status == 2">
              <icon :cover="row.fileCover" :width="36"></icon>
            </template>
            <template v-else>
            ...
            <span class="op">
              <template v-if="row.showOp && row.fileId && row.status == 2">
                ...
                <span class="iconfont icon-share1" @click="share(row)"></span>
                </el-tooltip>
                <el-tooltip content="下载" placement="bottom">
                <span class="iconfont icon-download" @click="download(row)" v-if="row.folderType == 0"></span>
                </el-tooltip>
                <el-tooltip content="移动" placement="bottom">
                <span class="iconfont icon-move" @click="moveFolder(row)"></span>
              </el-tooltip>
                <el-tooltip content="重命名" placement="bottom">
                <span class="iconfont icon-edit" @click.stop="editFileName(index)"></span>
                </el-tooltip>
                <el-tooltip content="删除" placement="bottom">
                <span class="iconfont icon-del" @click="delFile(row)"></span>
                </el-tooltip>
              </template>
            </span>
          </div>
    ...
    <div class="no-data" v-else>
      <div class="no-data-inner" style="border: 10px solid pink;">
